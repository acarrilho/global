...
namespace Global.FtpClient
{
    ...
    public class FtpClient
    {
        ...
        /// <param name="resume">To sup</param>
        /// <returns></returns>
        /// <remarks>If the target filename is blank, the source filename is used
        /// (assumes current directory). Otherwise use a filename to specify a name
        /// or a full path and filename if required.</remarks>
        public bool Upload(string localFilename, string targetFilename, bool resume)
        {
            //1. check source
            ...
            return Upload(fi, targetFilename, resume);
        }
        ...
        /// <returns></returns>
        public bool Upload(FileInfo fi, string targetFilename, bool resume)
        {
            ...
            using (FileStream fs = fi.OpenRead())
            {
                ...
                {
                    throw new WebException(((FtpWebResponse)e.Response).StatusDescription);
                }
                catch (Exception ex)
                {
    ...
    public class FtpIO
    {
        public static string ValidateStartOfPath(string path)
        {
            string validated = "";

            if (path.StartsWith("ftp://"))
            {
                validated = path.Replace("ftp://", "");
                validated = validated.Substring(validated.IndexOf("/"), validated.Length - validated.IndexOf("/"));
            }
